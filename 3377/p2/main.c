...
int main(int argc, char** argv)
{
    ...
    arr = mmap(NULL, sb.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
...
//Psudeo code for hash tree.
/*
Create N threads(nodes) for the amt given by the argv[2] and assign its necessary
block to hash. blocks to compute is i* n/m, i being the thread #. once
children are done hashing, send to parents, hash, concat, and rehash. repeat.


DONT CREATE A PERFECT BINARY TREE (DONT NEED ALL LEAF NODES TO BE PRESENT)
    - account for total threads created so far.
...
void* tree(void* arg) 
{
	...
	char buffer[60]; // 60 is enough for all max uin32 as strings
	...
	if(shouldSpawnLeft && shouldSpawnRight) {
		int length = sprintf(buffer, "%u%u%u", hash, *(uint32_t*)resultLeft, *(uint32_t*)resultRight);
		*totalPtr = calcHash((uint8_t*) buffer, length);
		free(resultLeft);
		free(resultRight);
	}
	else if (shouldSpawnLeft) {
		int length = sprintf(buffer, "%u%u", hash, *(uint32_t*)resultLeft);
		*totalPtr = calcHash((uint8_t*) buffer, length);
		free(resultLeft);
	}
	else {
